...
class AnimatedButton(ctk.CTkButton):
    def __init__(self, parent, light_path, dark_path):
        ...
        super().__init__(parent, text='An Animated Button', image=self.frames[self.frame_index], command=self.trigger_animation)
    ...
    def import_folders(self, light_path, dark_path):
        ...
        for path in (light_path, dark_path):
            for _, __, image_data in walk(path):
                sorted_data = sorted(image_data, key=lambda item: int(item.split('.')[0][-5:]))
        ...
        for image_path in image_paths:
            ctk_image = ctk.CTkImage(light_image=Image.open(image_path[0]), dark_image=Image.open(image_path[1]))
...
window = ctk.CTk()
window.title('Image Animations')
window.geometry('400x200+450+150')
